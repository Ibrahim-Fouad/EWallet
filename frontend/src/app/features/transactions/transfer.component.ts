...
type Step = 'form' | 'confirm' | 'success' | 'fail';
        ...
        <div class="page" style="max-width: 560px; margin: 48px auto">
          <div class="card" style="padding: 40px; text-align: center">
            <div class="success-icon">
              <app-icon name="check" [size]="36" [strokeWidth]="2.5" />
            </div>
            <h2 class="t-h1" style="margin-top: 24px">Transfer sent!</h2>
            ...
            @if (resultTx(); as tx) {
              ...
              >
                <div class="detail-row">
                  ...
                  <span class="val t-mono t-small">{{ tx.id }}</span>
                </div>
                <div class="detail-row">
        ...
        <div class="page" style="max-width: 720px; margin: 0 auto">
          ...
          <div class="row gap-4" style="align-items: flex-start">
            ...
            <div class="card card-pad col gap-3" style="flex: 1; position: sticky; top: 96px">
              ...
              <label
                class="row gap-2"
                style="align-items: center; margin-top: 4px; cursor: pointer"
              >
                <input
                  type="checkbox"
                  [checked]="simFail()"
                  (change)="onSimFailChange($event)"
                />
                <span class="t-tiny secondary">Demo: simulate a failed transfer</span>
              </label>
            </div>
          </div>
          ...
          >
            ...
            <div modal-footer class="modal-footer">
              <button type="button" class="btn btn-secondary" (click)="backToForm()">Cancel</button>
              <button type="button" class="btn btn-primary" (click)="confirmSend()">
...
export class TransferComponent {
  ...
  protected readonly resultTx = signal<Transaction | null>(null);
  protected readonly failReason = signal<string>('');
  protected readonly simFail = signal(false);
  ...
  protected readonly recipientLabel = computed(
  ...
  protected onSimFailChange(event: Event): void {
    this.simFail.set((event.target as HTMLInputElement).checked);
  }
  ...
  protected confirmSend(): void {
    ...
    if (!fromId) return;
    const r = this.state.transfer({
      fromWalletId: fromId,
      toPhone: this.toPhone(),
      amount: this.amountNum(),
      simulateFail: this.simFail(),
    });
    if (r.ok) {
      this.resultTx.set(r.tx);
      this.step.set('success');
      this.state.pushToast({
        kind: 'success',
        title: 'Transfer completed',
        body: `${this.formattedAmount()} sent`,
      });
    } else {
      this.resultTx.set(r.tx ?? null);
      this.failReason.set(r.reason);
      this.step.set('fail');
      this.state.pushToast({
        kind: 'error',
  ...
  protected backToForm(): void {
    this.step.set('form');
    this.simFail.set(false);
  }

  protected reset(): void {
    this.step.set('form');
    this.resultTx.set(null);
    this.amount.set('');
    this.toPhone.set('');
    this.note.set('');
    this.errors.set({});
